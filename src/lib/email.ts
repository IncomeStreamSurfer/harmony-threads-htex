import { orderConfirmationHtml, contactAckHtml, subscribeConfirmHtml } from "./email-templates";
...
export async function sendSubscribeConfirm(args: { to: string }) {
  return sendEmail({
    to: args.to,
    subject: "You're on the Harmony Threads list",
    html: subscribeConfirmHtml({ email: args.to }),
    tags: [{ name: "type", value: "subscribe_confirm" }],
  });
}
